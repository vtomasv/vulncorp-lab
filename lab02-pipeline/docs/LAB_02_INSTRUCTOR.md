...
1c. **Contraseña de Dependency-Track**: En la versión 4.14+, el primer login con `admin/admin` requiere un cambio de contraseña obligatorio. El script `upload_reports.py` maneja esto automáticamente vía API y cambia la contraseña a `VulnCorp2026!`. Si los estudiantes intentan entrar a la UI antes de correr el script, tendrán que cambiarla manualmente y luego el script podría fallar si no usan esa misma contraseña.
